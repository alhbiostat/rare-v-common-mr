...
# Date: 20-02-2026
# Author: A.L.Hanson
# Purpose: Prune common WES variants used as O'link protein instruments (pQTL) based on LD in UKB EUR reference (for pleiotropy analysis)
...
# Run:
# run using: ./001d_prunecommonexwasIVs.sh

# Inputs:
# commoninstruments_p5e-8.tsv (list of rare protein pQTLs)
...
# Output:
# wes_pQTLs_common_chr${i}.prune.in .prune.out (independent variants to retain)
...
for i in {1..22}; do
    ...
    # Variant IDs
    awk -F'\t' 'NR>1 && \$2==${i} {print \$1}' commoninstruments_p5e-8.tsv | sort | uniq > varset.txt
    ...
    --out wes_pQTLs_common_p5e-8_chr${i}
    ...
    -iin="${rap_data_dir}/WES/pqtls/commoninstruments_p5e-8.tsv" \
    -icmd="${run_prune}" \
    --tag="prune common pqtls chr${i}" \
